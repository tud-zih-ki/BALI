...
parser.add_argument("--model_name", default="microsoft/llmlingua-2-xlm-roberta-large-meetingbank")
parser.add_argument("--device", required=True)
parser.add_argument("--load_origin_from", required=True)
parser.add_argument("--load_key", help="the key to load the text to compress", default="prompt")
parser.add_argument("--save_key", help="the key to save the compressed text", default="compressed_prompt")
parser.add_argument("--save_path", required=True)

# for llmlingua2
parser.add_argument("--compression_rate", type=float, default=0.5)
parser.add_argument("--target_token", type=int, default=-1)
# llmlingua2 coarse to fine
parser.add_argument("--use_token_level_filter", action=argparse.BooleanOptionalAction, default=True)
...
# llmlingua2 details
parser.add_argument("--force_tokens", help="forcely preserved tokens, comma separated", type=str, default=None)
parser.add_argument("--drop_consecutive", action=argparse.BooleanOptionalAction, default=True)
parser.add_argument("--force_reserve_digit", action=argparse.BooleanOptionalAction, default=False)

args = parser.parse_args()
os.makedirs(os.path.dirname(args.save_path), exist_ok=True)
if args.force_tokens is not None:
    args.force_tokens = [str(item).replace("\\n", "\n") for item in args.force_tokens.split(",")]
else:
    args.force_tokens = []
...
for sample in tqdm(data):
    ...
    # Handle ZeroScrolls
    zero_scrolls = "doc_start" in sample
    if zero_scrolls:
        origin = origin[sample["doc_start"] : sample["doc_end"]]
    ...
    if zero_scrolls:
        len_diff = len(origin) - len(comp)
        comp = sample[args.load_key][: sample["doc_start"]] + comp + sample[args.load_key][sample["doc_end"] :]
        new_sample["doc_end"] = sample["doc_end"] - len_diff
        new_sample["query_start"] = sample["query_start"] - len_diff
        new_sample["query_end"] = sample["query_end"] - len_diff
...
json.dump(results, open(args.save_path, "w", encoding="utf8"), indent=4, ensure_ascii=False)
