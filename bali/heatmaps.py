...
def find_latest_results_dir():
    """Find the most recent bali_results/* directory."""
    results_dirs = glob.glob("bali_results/*")
...
def collect_data(results_dir, model_filter=None, framework_filter=None, batch_size_filter=None):
    ...
    # Traverse: model/framework/batch_size/input_len/output_len/
    for model_name in os.listdir(results_dir):
        ...
        for framework in os.listdir(model_path):
            ...
            for batch_size_str in os.listdir(framework_path):
                batch_size_path = os.path.join(framework_path, batch_size_str)
                ...
                if batch_size_filter is not None and batch_size != batch_size_filter:
                    continue

                for input_len_str in os.listdir(batch_size_path):
                    input_len_path = os.path.join(batch_size_path, input_len_str)
...
def create_heatmaps(results_dir=None, model_names=None, frameworks=None, batch_size=None, output_dir=None):
    """
    ...
    if results_dir is None:
        ...
        if results_dir is None:
            print("No results directories found (bali_results/*)")
            return
    ...
    data, available_models, available_frameworks = collect_data(results_dir, model_names, frameworks, batch_size)
