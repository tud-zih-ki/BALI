...
%bali_run                   - Run benchmarks for all parameter combinations
                             Results saved to: bali_results/<pid>/<idx>/
...
%bali_plot [options]        - Create performance heatmaps
    ...
    --batch_size BS          - Filter specific batch size

Examples:
    %bali_plot --batch_size 1
    %bali_plot --batch_size 4 --model_name facebook_opt-1.3b --frameworks vllm hf_accelerate
        """
    ...
    def bali_run(self, line):
        ...
        base_output_dir = f"bali_results/{self.pid}/{self.run_index}"
        ...
        for i, (model_name, framework, batch_size, input_len, output_len) in enumerate(combinations, 1):
            ...
            try:
                ...
                print(f"Error in combination {i+1}/{len(combinations)}: {str(e)}")
                continue
        ...
        print(f"Completed! Results saved to: {base_output_dir}")
        self.run_index += 1
    ...
    def bali_plot(self, line):
        ...
        try:
            ...
            print("Usage: %bali_plot [--output_dir DIR] [--model_name MODEL1 ...] [--frameworks FW1 ...] [--batch_size BS]")
        except Exception as e:
            print(f"Error: {e}")
